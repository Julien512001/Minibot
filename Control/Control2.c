...
#define M_PI  3.14
#define TS 0.01
// gcc -o control2 LR_control_2.c -lpigpio -lrt -lpthread -lm
...
void runMotors(int cycle, float step) {
    ...
    while (i < cycle) {
        if (i <= cycle/8) {
            target_speed_L = 10.0;
            target_speed_R = 10.0;
        } else if (i <= cycle/4 + 284*2 & i > cycle/4) {
            target_speed_L = 10.0;
            target_speed_R = -10.0;
        } else {
            target_speed_L = 10.0;
        ...
        controlSpeed(target_speed_L, target_speed_R, step);
